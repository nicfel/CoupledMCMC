...
public class MultiCoupledMCMC extends CoupledMCMC {
	public Input<Integer> m_nrOfChains = new Input<Integer>("multichains", " number of chains to run in parallel (default 2)", 2);
	
	/** plugins representing MCMC with model, loggers, etc **/
	CoupledMCMC [] m_chains;
	...
	public void initAndValidate() {
		m_chains = new CoupledMCMC[m_nrOfChains.get()];
		...
		for (int i = 0; i < m_chains.length; i++) {
			...
			try {
				m_chains[i] = (CoupledMCMC) parser.parseFragment(sXML2, true);
